...
| 문제 | 해결 레이어 | 해결 방법 |
|---|---|---|
| 맥락 유실·추적 불가 | 전 과정 추적 레이어 | Opencode 세션으로 작업를 관측, 개입하고 Sudocode로 이슈 상태를 추적하고, ADR로 의사결정 이유를 기록하여 모든 의사결정을 추적할 수 있다.  |
...
이슈 단위로 작업 상태를 관리하는 서버다. 에이전트와 사람이 같은 이슈 목록을 공유하며, AI 에이전트의 장기 기억 컨텍스트 역할을 한다.
...
- **컨텍스트 고립 (Context Isolation)** — 서브에이전트마다 신선한 컨텍스트 윈도우를 할당해 맥락 오염을 차단한다. 구현 에이전트가 쌓은 편향이 리뷰 에이전트에 흘러들어가지 않으므로 자기검증 편향이 사라진다. 구현과 리뷰를 별도 인스턴스에 파견하는 것이 이 고립을 구현하는 수단이다.
