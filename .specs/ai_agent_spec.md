...
**`bad_records_summary` 하드 캡 정책(ADR-0009 고정값)**:

- 위반 유형 상한: 최대 50개 (`type_count <= 50`)
- 유형별 샘플 상한: 최대 10개 (`len(samples) <= 10`)
...
- 폭주 억제 기준: 대량 입력(1만건+, 다유형, 긴 문자열)에서도 요약 직렬화 크기가 안정적으로 bounded 되어 analyze 입력으로 직접 전달 가능해야 한다.
- 재현 가능한 게이트: `tests/unit/test_bad_records_summarizer.py`의 경계/대량 테스트를 CI 회귀 게이트로 유지한다.

**상한값 조정 절차**:

1. 운영 데이터에서 폭주/정보손실 징후를 확인한다(요약 과대 또는 과소).
2. 0건/1건/1만건+ 경계 테스트 + 대량 직렬화 크기 테스트를 먼저 추가/수정해 실패를 재현한다.
3. 상한값 수정 후 동일 테스트를 통과시켜 회귀 기준을 갱신한다.
4. ADR-0009 및 본 스펙의 상한/절단 규칙을 동일 값으로 동기화한다.
...
환경변수 키 이름 검증의 코드 기준 경로는 ADR-0015에 따라 `src/orchestrator/utils/config.py`로 고정한다.
...
| 일일 호출 상한 | `LLM_DAILY_CAP` (논리 호출 단위 기본 30회/day, 환경변수 override) | 비용 통제 + 운영 환경별 조정 |
...
- **일일 LLM 호출 상한은 `LLM_DAILY_CAP`로 제어하며 기본값은 30회/day (논리 호출 기준)**. 정상 시 0회, 장애 시 analyze + triage + postmortem = 3회. 운영 환경에서는 장애 빈도/비용 예산에 맞춰 값을 조정한다. 상한 도달 시 추가 호출을 차단하고 이메일로 통보.
- 실제 HTTP 시도 수(재시도 포함)는 논리 호출 수와 별도로 로그를 통해 관측한다.
